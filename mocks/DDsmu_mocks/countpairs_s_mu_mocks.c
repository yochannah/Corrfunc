...
int countpairs_mocks_s_mu(const int64_t ND1, void *phi1, void *theta1, void *czD1,
                          ...
                          const int numthreads, const int autocorr, const char *sbinfile,
                          const double mu_max, const int nmu_bins, const int cosmology,
                          results_countpairs_mocks_s_mu *results, struct config_options *options,
                          struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countpairs_mocks_s_mu_float(ND1, (float *)phi1, (float *)theta1, (float *)czD1, ND2,
                                       (float *)phi2, (float *)theta2, (float *)czD2, numthreads,
                                       autocorr, sbinfile, mu_max, nmu_bins, cosmology, results,
                                       options, extra);
  } else {
    return countpairs_mocks_s_mu_double(ND1, (double *)phi1, (double *)theta1, (double *)czD1, ND2,
                                        (double *)phi2, (double *)theta2, (double *)czD2,
                                        numthreads, autocorr, sbinfile, mu_max, nmu_bins, cosmology,
                                        results, options, extra);
  }
}
