...
--- DDsmu file1 format1 file2 format2 sbinfile Nmu cosmology numthreads [weight_method weights_file1
weights_format1 [weights_file2 weights_format2]] > DDfile
 ...
 * cosmology     = flag to pick-up the cosmology combination to use (set as an array of combinations
in ../utils/cosmology_params.c)
...
int main(int argc, char *argv[])
{
  ...
  /*---Corrfunc-variables----------------*/
#if defined(_OPENMP)
  const char argnames[][30] = {"file1",  "format1",  "file2",          "format2",   "sbinfile",
                               "mu_max", "nmu_bins", "cosmology flag", "numthreads"};
#else
  const char argnames[][30] = {"file1",    "format1", "file2",    "format2",
                               "sbinfile", "mu_max",  "nmu_bins", "cosmology flag"};
#endif
  ...
  int status = countpairs_mocks_s_mu(ND1, phiD1, thetaD1, czD1, ND2, phiD2, thetaD2, czD2, nthreads,
                                     autocorr, sbinfile, mu_max, nmu_bins, cosmology, &results,
                                     &options, &extra);
  ...
  for (int i = 1; i < results.nsbin; i++) {
    ...
    for (int j = 0; j < nmubin; j++) {
      ...
      fprintf(stdout, "%10" PRIu64 " %20.8lf %20.8lf  %20.8lf %20.8lf \n", results.npairs[index],
              results.savg[index], log_supp, (j + 1) * dmu, results.weightavg[index]);
    }
  }
  ...
  fprintf(stderr,
          "DDsmu> Done -  ND1=%" PRId64 " ND2=%" PRId64
          ". Time taken = %6.2lf seconds, read-in time = %6.2lf seconds \n",
...
void Printhelp(void)
{
  fprintf(stderr, "=========================================================================\n");
#if defined(USE_OMP) && defined(_OPENMP)
  fprintf(stderr, "   --- DDsmu file1 format1 file2 format2 sbinfile nmu_bins mu_max cosmology "
                  "numthreads [weight_method weights_file1 weights_format1 [weights_file2 "
                  "weights_format2]] > DDfile\n");
#else
  fprintf(stderr, "   --- DDsmu file1 format1 file2 format2 sbinfile nmu_bins mu_max cosmology "
  ...
  fprintf(stderr, "     * cosmology     = flag to pick-up the cosmology combination to use (set as "
                  "an array of combinations in ../utils/cosmology_params.c)\n");
#if defined(USE_OMP) && defined(_OPENMP)
  ...
  fprintf(
      stderr,
      "     > DDfile        = name of output file. Contains <npairs savg logrp mu weightavg>\n");
