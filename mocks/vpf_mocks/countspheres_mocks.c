...
int countspheres_mocks(const int64_t Ngal, void *xgal, void *ygal, void *zgal, const int64_t Nran,
                       void *restrict xran, void *restrict yran, void *restrict zran,
                       const int threshold_neighbors, const double rmax, const int nbin,
                       const int nc, const int num_pN, const char *centers_file,
                       const int cosmology, results_countspheres_mocks *results,
                       struct config_options *options, struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countspheres_mocks_float(Ngal, (float *)xgal, (float *)ygal, (float *)zgal, Nran,
                                    (float *)xran, (float *)yran, (float *)zran,
                                    threshold_neighbors, rmax, nbin, nc, num_pN, centers_file,
                                    cosmology, results, options, extra);
  } else {
    return countspheres_mocks_double(Ngal, (double *)xgal, (double *)ygal, (double *)zgal, Nran,
                                     (double *)xran, (double *)yran, (double *)zran,
                                     threshold_neighbors, rmax, nbin, nc, num_pN, centers_file,
                                     cosmology, results, options, extra);
  }
}
