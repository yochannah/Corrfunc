...
/* PROGRAM vpf_mocks
   ...
   --- vpf_mocks rmax nbin nc num_pN volume mocksfile mocksformat RANDfile randformat centers-file
   cosmology > output
   ...
   * numpN              = number of counts-in-spheres to output. [numpN=1-> P0, numpN=2->P0,P1,
   numpN=3->P0,P1,P2...\n");
   ...
   * centers file       = file containing sphere centers (XYZ). If there are not enough centers,
   file will be truncated and re-written with centers
   * cosmology          = flag to pick-up the cosmology combination to use (set as an array of
   combinations in ../utils/cosmology_params.c) > output: <R P0 P1 P2 ...>
*/
...
int main(int argc, char *argv[])
{
  ...
  if (need_randoms == 1) {
    ...
    threshold_neighbors =
        (int)(Nexpected - sqrt(Nexpected));    // allow 1-sigma deviation. Conservative
  } else {
    threshold_neighbors = 1;    // dummy value -> just to ensure that the check does not compare
                                // with uninitialized values
    Nran = nc;    // HACK: set Nran to number of spheres requested. Code will not execute loop
                  // otherwise
  }
  ...
  int status =
      countspheres_mocks(Ngal, ra, dec, cz, Nran, xran, yran, zran, threshold_neighbors, rmax, nbin,
                         nc, num_pN, centers_file, cosmology, &results, &options, NULL);
...
{
  fprintf(stderr, "=========================================================================\n");
  fprintf(stderr, "   --- vpf_mocks rmax nbin nc num_pN volume mocksfile mocksformat RANDfile "
                  "randformat centers-file cosmology > output\n");
  ...
  fprintf(stderr, "      * numpN        = number of counts-in-spheres to output. [numpN=1-> P0, "
                  "numpN=2->P0,P1, numpN=3->P0,P1,P2...\n");
  ...
  fprintf(stderr, "      * file with sphere centers (centers will be read-in if enough centers "
                  "exist, otherwise centers will be output into this file)\n");
