...
int test_DDrppi_mocks(const char *correct_outputfile)
{
  ...
  assert(RA1 != NULL && DEC1 != NULL && CZ1 != NULL &&
         "ERROR: In test suite for DDrppi ra/dec/cz can not be NULL pointers");
  ...
  int status = countpairs_mocks(ND1, RA1, DEC1, CZ1, ND2, RA2, DEC2, CZ2, nthreads, autocorr,
                                binfile, pimax, cosmology_flag, &results, &options, &extra);
  ...
  for (int i = 1; i < results.nbin; i++) {
    for (int j = 0; j < npibin; j++) {
      ...
      int floats_equal =
          AlmostEqualRelativeAndAbs_double(rpavg, results.rpavg[index], maxdiff, maxreldiff);
      int weights_equal = AlmostEqualRelativeAndAbs_double(weightavg, results.weightavg[index],
                                                           maxdiff, maxreldiff);
    ...
    for (int i = 1; i < results.nbin; i++) {
      ...
      for (int j = 0; j < npibin; j++) {
        ...
        fprintf(fp, "%10" PRIu64 " %20.8lf %20.8lf  %20.8lf %20.8lf \n", results.npairs[index],
                results.rpavg[index], logrp, (j + 1) * dpi, results.weightavg[index]);
      }
    }
    fclose(fp);
  }
...
int test_DDsmu_mocks(const char *correct_outputfile)
{
  ...
  assert(RA1 != NULL && DEC1 != NULL && CZ1 != NULL &&
         "ERROR: In test suite for DDsmu ra/dec/cz can not be NULL pointers");
  int autocorr = (RA1 == RA2) ? 1 : 0;
  ...
  // Do DD(s,mu) counts
  int status =
      countpairs_mocks_s_mu(ND1, RA1, DEC1, CZ1, ND2, RA2, DEC2, CZ2, nthreads, autocorr, binfile,
                            mocks_mu_max, nmu_bins, cosmology_flag, &results, &options, &extra);
  if (status != EXIT_SUCCESS) {
    return status;
  }
  ...
  for (int i = 1; i < results.nsbin; i++) {
    for (int j = 0; j < nmubin; j++) {
      ...
      int floats_equal =
          AlmostEqualRelativeAndAbs_double(savg, results.savg[index], maxdiff, maxreldiff);
      int weights_equal = AlmostEqualRelativeAndAbs_double(weightavg, results.weightavg[index],
                                                           maxdiff, maxreldiff);
      ...
      if (npairs == results.npairs[index] && floats_equal == EXIT_SUCCESS &&
        ...
        fprintf(stderr, "True savg  = %20.12e Computed savg = %20.12e. floats_equal = %d\n", savg,
                results.savg[index], floats_equal);
        fprintf(stderr, "True weightavg = %e Computed weightavg = %e. weights_equal = %d\n",
                weightavg, results.weightavg[index], weights_equal);
        ret = EXIT_FAILURE;    // not required but showing intent
        i = results.nsbin;
  ...
  if (ret != EXIT_SUCCESS) {
    ...
    for (int i = 1; i < results.nsbin; i++) {
      ...
      for (int j = 0; j < nmubin; j++) {
        int index = i * (nmubin + 1) + j;
        fprintf(fp, "%10" PRIu64 " %20.8lf %20.8lf  %20.8lf %20.8lf \n", results.npairs[index],
                results.savg[index], logrp, (j + 1) * dmu, results.weightavg[index]);
      }
    }
    fclose(fp);
  }
...
int test_DDtheta_mocks(const char *correct_outputfile)
{
      ...
      for (int ra_link = 0; ra_link <= dec_link; ra_link++) {
        ...
        for (int ra_bin_ref = min_bin_ref; ra_bin_ref <= max_bin_ref; ra_bin_ref++) {
          for (int dec_bin_ref = min_bin_ref; dec_bin_ref <= max_bin_ref; dec_bin_ref++) {
            ...
            if (dotest == 1) {
              if (dec_link == 0 && ra_link == 0)
                continue;    // I have checked the brute force

              // bin refine factor has no impact on brute-force -> only check brute-force once
              if (dec_link == 0 && ra_link == 0 &&
                  (dec_bin_ref != min_bin_ref || ra_bin_ref != min_bin_ref))
                continue;
              ...
              fprintf(stderr,
                      "Running with dec-linking = %d ra-linking = %d bin-ref = (%d, %d) and "
                      "instruction set = %s ",
                      dec_link, ra_link, options.bin_refine_factors[0],
                      options.bin_refine_factors[1], isa_name[iset]);
...
#else
  {
    ...
    options.link_in_ra = 1;
              ...
              int status =
                  countpairs_theta_mocks(ND1, RA1, DEC1, ND2, RA2, DEC2, nthreads, autocorr,
                                         angular_binfile, &results, &options, &extra);
              ...
              for (int i = 1; i < results.nbin; i++) {
                ...
                int floats_equal = AlmostEqualRelativeAndAbs_double(theta_avg, results.theta_avg[i],
                                                                    maxdiff, maxreldiff);
                int weights_equal = AlmostEqualRelativeAndAbs_double(
                    weightavg, results.weightavg[i], maxdiff, maxreldiff);
                ...
                if (npairs == results.npairs[i] && floats_equal == EXIT_SUCCESS &&
                  ...
                  ret = EXIT_FAILURE;    // not required but showing intent
                  fprintf(stderr,
                          "Failed. True npairs = %" PRIu64 " Computed results npairs = %" PRIu64
                          "\n",
                          npairs, results.npairs[i]);
                  fprintf(stderr,
                          "Failed. True thetaavg = %e Computed thetaavg = %e. floats_equal = %d\n",
                          theta_avg, results.theta_avg[i], floats_equal);
                  fprintf(
                      stderr,
                      "Failed. True weightavg = %e Computed weightavg = %e. floats_equal = %d\n",
        ...
        if (ret == EXIT_SUCCESS) {
          fprintf(stderr,
                  ANSI_COLOR_MAGENTA "Fastest time = %8.2lf seconds with bin-ref = {%d, %d} and "
                                     "instruction_set = %s" ANSI_COLOR_RESET "\n",
                  fastest_time * 1e-9, fastest_bin_ref[0], fastest_bin_ref[1],
  ...
  if (ret != EXIT_SUCCESS) {
    ...
    for (int i = 1; i < results.nbin; i++) {
      fprintf(fp, "%10" PRIu64 " %20.8lf %20.8lf %20.8lf %20.8lf\n", results.npairs[i],
              results.theta_avg[i], theta_low, results.theta_upp[i], results.weightavg[i]);
...
int test_vpf_mocks(const char *correct_outputfile)
{
  ...
  BEGIN_INTEGRATION_TEST_SECTION
  int status =
      countspheres_mocks(ND1, RA1, DEC1, CZ1, Nran, xran, yran, zran, threshold_neighbors, rmax,
                         nbin, nc, num_pN, centers_file, cosmology_flag, &results, &options, NULL);
  if (status != EXIT_SUCCESS) {
    return status;
  }
  ...
  for (int ibin = 0; ibin < results.nbin; ibin++) {
    ...
    for (int i = 0; i < num_pN; i++) {
      ...
      /* Not quite sure how this is working. The correct output columns only have 4 digits printed,
         ...
         if the conversion to 4 digits during printf, round-trips during scanf. But surely there
         must be a lot more doubles that can be fit within those missing digits of precision.

         I would have thought the comparison would require maxdiff ~ 1e-4. -- MS
      */
      int floats_equal =
          AlmostEqualRelativeAndAbs_double(pN, (results.pN)[ibin][i], maxdiff, maxreldiff);
                               ...
                               const char *secondfilename, const char *secondformat)
{
  ...
  if (strcmp(current_file1, firstfilename) != 0) {
    /* fprintf(stderr,"Freeing the first data-set and replacing with data from file
     * `%s'\n",firstfilename); */
    ...
    ND1 =
        read_positions(firstfilename, firstformat, sizeof(double), 4, &RA1, &DEC1, &CZ1, &weights1);
  }
  ...
  if (strncmp(firstfilename, secondfilename, strlen(firstfilename)) == 0) {
    ...
    /* fprintf(stderr,"Second data-set is the same as the first data-set. First file = `%s' and
  ...
  if (strncmp(current_file2, secondfilename, strlen(current_file2)) != 0 || RA2 == NULL) {
    ...
    /* fprintf(stderr,"Second data-set is different -- reading in the new data-set from
     * `%s'\n",secondfilename); */
    ND2 = read_positions(secondfilename, secondformat, sizeof(double), 4, &RA2, &DEC2, &CZ2,
                         &weights2);
    strncpy(current_file2, secondfilename, MAXLEN);
  }
}
...
int main(int argc, char **argv)
{
  ...
  const char alltests_names[][MAXLEN] = {"Mr19 mocks DDrppi (DD)", "Mr19 mocks wtheta (DD)",
                                         "Mr19 mocks vpf (data)",  "Mr19 mocks DDrppi (DR)",
                                         "Mr19 mocks wtheta (DR)", "Mr19 mocks vpf (randoms)",
                                         "Mr19 mocks DDsmu (RR)",  "Mr19 mocks DDsmu (DR)"};
  const int ntests = sizeof(alltests_names) / (sizeof(char) * MAXLEN);
  const int function_pointer_index[] = {0, 1, 2, 0,
                                        1, 2, 3, 3};    // 0->DDrppi, 1->wtheta, 2->vpf, 3->DDsmu
  assert(sizeof(function_pointer_index) / sizeof(int) == ntests &&
         "Number of tests should equal the number of functions");

  const char correct_outputfiles[][MAXLEN] = {
      "../tests/Mr19_mock.DD",        /* Test 0 Mr19 DD */
  ...
  int (*allfunctions[])(const char *) = {test_DDrppi_mocks, test_DDtheta_mocks, test_vpf_mocks,
                                         test_DDsmu_mocks};
  ...
  if (argc == 1) {
    ...
    for (int i = 0; i < ntests; i++) {
      ...
      assert(function_index >= 0 && function_index < numfunctions &&
             "Function index is within range");
      const char *testname = alltests_names[i];
      int skip_test = test_all_files_present(2, firstfilename[i], secondfilename[i]);
      if (skip_test != 0) {
        fprintf(stderr,
                ANSI_COLOR_YELLOW "SKIPPED: " ANSI_COLOR_MAGENTA "%s" ANSI_COLOR_RESET
                                  ". Test data-file(s) (`%s',`%s') not found\n",
                testname, firstfilename[i], secondfilename[i]);
        skipped++;
        continue;
      }
      read_data_and_set_globals(firstfilename[i], firstfiletype[i], secondfilename[i],
                                secondfiletype[i]);
      pimax = allpimax[i];
      ...
      if (status == EXIT_SUCCESS) {
        fprintf(stderr,
                ANSI_COLOR_GREEN "PASSED: " ANSI_COLOR_MAGENTA "%s" ANSI_COLOR_GREEN
                                 ". Time taken = %8.2lf seconds " ANSI_COLOR_RESET "\n",
                testname, pair_time);
        ...
        fprintf(stderr,
                ANSI_COLOR_RED "FAILED: " ANSI_COLOR_MAGENTA "%s" ANSI_COLOR_RED
                               ". Time taken = %8.2lf seconds " ANSI_COLOR_RESET "\n",
                testname, pair_time);
        failed++;
        char execstring[MAXLEN];
        ...
        fprintf(stderr,
                ANSI_COLOR_RED
                "Failed output copied to %s.%d correct output is in %s" ANSI_COLOR_RESET "\n",
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (this_test_num >= 0 && this_test_num < ntests) {
        ...
        int skip_test =
            test_all_files_present(2, firstfilename[this_test_num], secondfilename[this_test_num]);
        if (skip_test != 0) {
          fprintf(stderr,
                  ANSI_COLOR_YELLOW "SKIPPED: " ANSI_COLOR_MAGENTA "%s" ANSI_COLOR_RESET
                                    ". Test data-file(s) (`%s',`%s') not found\n",
        ...
        total_tests++;
        read_data_and_set_globals(firstfilename[this_test_num], firstfiletype[this_test_num],
                                  secondfilename[this_test_num], secondfiletype[this_test_num]);
        ...
        if (status == EXIT_SUCCESS) {
          ...
          failed++;
          char execstring[MAXLEN];
          my_snprintf(execstring, MAXLEN, "mv %s %s.%d", tmpoutputfile, tmpoutputfile,
                      this_test_num);
          run_system_call(execstring);    // ignoring status
          fprintf(stderr,
                  ANSI_COLOR_RED
        ...
        fprintf(stderr,
                ANSI_COLOR_YELLOW "WARNING: Test = %d is not a valid test index. Valid test "
                                  "indices range between [0,%d] " ANSI_COLOR_RESET "\n",
  ...
  if (failed > 0) {
    fprintf(stderr,
            ANSI_COLOR_RED
            "FAILED %d out of %d tests. Total time = %8.2lf seconds " ANSI_COLOR_RESET "\n",
            failed, total_tests, total_time);
  } else {
    fprintf(stderr,
            ANSI_COLOR_GREEN "PASSED: ALL %d tests. Total time = %8.2lf seconds " ANSI_COLOR_RESET
                             "\n",
            total_tests, total_time);
  }
  if (skipped > 0) {
    fprintf(stderr, ANSI_COLOR_YELLOW "SKIPPED: %d tests" ANSI_COLOR_RESET "\n", skipped);
    fprintf(stderr, ANSI_COLOR_MAGENTA "Tests are skipped on the PyPI installed code-base. Please "
                                       "use the git repo if you want to run the entire suite of "
                                       "tests" ANSI_COLOR_RESET "\n\n");
  }
