...
int countpairs_theta_mocks(const int64_t ND1, void *phi1, void *theta1, const int64_t ND2,
                           void *phi2, void *theta2, const int numthreads, const int autocorr,
                           const char *binfile, results_countpairs_theta *results,
                           struct config_options *options, struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countpairs_theta_mocks_float(ND1, (float *)phi1, (float *)theta1, ND2, (float *)phi2,
                                        (float *)theta2, numthreads, autocorr, binfile, results,
                                        options, extra);
  } else {
    return countpairs_theta_mocks_double(ND1, (double *)phi1, (double *)theta1, ND2, (double *)phi2,
                                         (double *)theta2, numthreads, autocorr, binfile, results,
                                         options, extra);
  }
}
