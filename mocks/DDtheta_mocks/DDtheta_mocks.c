...
/*
--- DDtheta file1 format1 file2 format2 binfile numthreads [weight_method weights_file1
weights_format1 [weights_file2 weights_format2]] > Thetafile
...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < results.nbin; i++) {
    fprintf(stdout, "%20.8lf %20.8lf %20.8lf %10" PRIu64 " %20.8lf\n", theta_low,
            results.theta_upp[i], results.theta_avg[i], results.npairs[i], results.weightavg[i]);
  ...
  gettimeofday(&t1, NULL);
  fprintf(stderr,
          "DDtheta> Done -  ND1=%" PRId64 " ND2=%" PRId64 ". Time taken = %6.2lf seconds. read-in "
                                                          "time = %6.2lf seconds sec pair-counting "
                                                          "time = %6.2lf sec\n",
...
void Printhelp(void)
{
  fprintf(
      ...
      "========================================================================================\n");
#if defined(_OPENMP)
  fprintf(stderr, "   --- DDtheta file1 format1 file2 format2 binfile numthreads [weight_method "
                  "weights_file1 weights_format1 [weights_file2 weights_format2]] > Thetafile\n");
#else
  fprintf(stderr, "   --- DDtheta file1 format1 file2 format2 binfile [weight_method weights_file1 "
                  "weights_format1 [weights_file2 weights_format2]] > Thetafile\n");
#endif
  ...
  fprintf(stderr, "     * binfile     = name of ascii file containing the theta-bins (thetamin "
                  "thetamax for each bin)\n");
#if defined(_OPENMP)
  ...
  fprintf(
      stderr,
      "     > Thetafile   = name of output file < thetamin thetamax thetaavg npairs weightavg >\n");
#else
  fprintf(stderr, "     > Thetafile   = name of output file <thetamin thetamax thetaavg=0.0 npairs "
                  "weightavg >\n");
#endif
  fprintf(
      stderr,
  ...
  /* #OPT += -DFAST_ACOS ## replaces acos by an 8th order REMEZ polynomial. Results are approximate
   * (max. absolute error 3.6e-9) ~50% boost, but obviously approximate */
