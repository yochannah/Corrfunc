...
int main(int argc, char **argv)
{
  ...
  const char argnames[][30] = {"file",      "format", "binfile",  "pimax",
                               "cosmology", "mu_max", "nmu_bins", "Nthreads"};
#else
  const char argnames[][30] = {"file",      "format", "binfile", "pimax",
                               "cosmology", "mu_max", "nmu_bins"};
#endif
  ...
  // Do the DD(rp, pi) counts
  {
    ...
    int status = countpairs_mocks(ND1, ra1, dec1, cz1, ND2, ra2, dec2, cz2, nthreads, autocorr,
                                  binfile, pimax, cosmology, &results, &options, NULL);
    if (status != EXIT_SUCCESS) {
      return status;
    }
  ...
  // Do the DD(s, mu) counts
  {
    ...
    fprintf(stderr,
            ...
            "../DDsmu_mocks/DDsmu_mocks", file, fileformat, file, fileformat, binfile, mu_max,
            nmu_bins, cosmology, nthreads);
#else
    fprintf(stderr,
            ANSI_COLOR_MAGENTA "Command-line for running equivalent DD(s,mu) calculation would "
                               "be:\n `%s %s %s %s %s %s %lf %d %d'" ANSI_COLOR_RESET "\n",
    ...
    int status =
        countpairs_mocks_s_mu(ND1, ra1, dec1, cz1, ND2, ra2, dec2, cz2, nthreads, autocorr, binfile,
                              mu_max, nmu_bins, cosmology, &results, &options, NULL);
    if (status != EXIT_SUCCESS) {
      return status;
    }
  ...
  // Do the DD(theta) counts
  {
...
#if defined(_OPENMP)
    fprintf(stderr,
            ANSI_COLOR_MAGENTA "Command-line for running equivalent w(theta) calculation would "
    ...
    int status = countpairs_theta_mocks(ND1, ra1, dec1, ND2, ra2, dec2, nthreads, autocorr, binfile,
...
#endif
    fprintf(stderr,
            ANSI_COLOR_GREEN "Done wtheta. Ngalaxies = %12" PRId64
                             " Time taken = %8.2lf seconds" ANSI_COLOR_RESET "\n",
            ND1, pair_time);
  ...
  // Do the VPF
  {
    ...
    fprintf(stderr,
            ANSI_COLOR_MAGENTA
            "Command-line for running equivalent DD(theta) calculation would be:\n"
            "`%s %lf %d %d %d %lf %s %s %s %s %s %d'" ANSI_COLOR_RESET "\n",
            "../vpf_mocks/vpf_mocks", rmax, nbin, nc, num_pN, 0.0, file, fileformat, "junk",
            "junkformat", centers_file, cosmology);

    results_countspheres_mocks results;
    int status =
        countspheres_mocks(ND1, ra1, dec1, cz1, Nran, xran, yran, zran, threshold_neighbors, rmax,
                           nbin, nc, num_pN, centers_file, cosmology, &results, &options, NULL);
    if (status != EXIT_SUCCESS) {
      return status;
    }
