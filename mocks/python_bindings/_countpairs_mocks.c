...
static char module_docstring[] =
    "Python extensions for calculating clustering statistics on MOCKS (spherical geometry).\n"
    "\n"
    "countpairs_rp_pi_mocks: Calculate the 2-D DD(" RP_CHAR "," PI_CHAR
    ") auto/cross-correlation function given two sets of ra/dec/cz and ra/dec/cz arrays.\n"
    "countpairs_theta_mocks: Calculate DD(theta) auto/cross-correlation function given two sets of "
    "ra/dec/cz and ra/dec/cz arrays.\n"
...
static PyMethodDef module_methods[] = {
    {"countpairs_rp_pi_mocks", (PyCFunction)countpairs_countpairs_rp_pi_mocks,
     ...
     "\n"
     "Calculate the 2-D pair-counts, " XI_CHAR "(" RP_CHAR ", " PI_CHAR
     "), auto/cross-correlation function given two\n"
     "sets of RA1/DEC1/CZ1 and RA2/DEC2/CZ2 arrays. This module is suitable for mock catalogs that "
     "have been\n"
     "created by carving out a survey footprint from simulated data. The module can also be used "
     "for actual\n"
     "observed galaxies, but you probably want to attach weights to the points to account for "
     "completeness etc.\n"
     ...
     "" XI_CHAR "(" RP_CHAR ", " PI_CHAR
     "). See the `~Corrfunc.utils.convert_3d_counts_to_cf.py and \n"
     ...
     "    `DDrppi_mocks/countpairs_rp_pi_mocks_impl.c.src`) and multiplies by the speed of light "
     "if\n"
     ...
     "weights2\n : array-like, real (float/double), shape (n_particles,) or "
     "(n_weights_per_particle,n_particles), optional\n"
     "   Weights for computing a weighted pair count."
     ...
     "If ``output_rpavg`` is not set then ``rpavg`` will be set to 0.0 for all bins; similarly for "
     "``weight_avg``. \n"
     ...
     ">>> ra,dec,cz = "
     "np.genfromtxt('../mocks/tests/data/"
     "Mr19_mock_northonly.rdcz.dat',dtype=np.float,unpack=True)\n"
     ">>> cosmology=1\n"
     ">>> autocorr=1\n"
     ">>> nthreads=4\n"
     ...
     "                                            verbose=True)\n"
     "\n"},
    {"countpairs_s_mu_mocks", (PyCFunction)countpairs_countpairs_s_mu_mocks,
     METH_VARARGS | METH_KEYWORDS,
     ...
     "                       c_api_timer=False, isa=-1)\n"
     "\n"
     "Calculate the 2-D pair-counts, " XI_CHAR "(s, " MU_CHAR
     "), auto/cross-correlation function given two\n"
     "sets of RA1/DEC1/CZ1 and RA2/DEC2/CZ2 arrays. This module is suitable for mock catalogs that "
     "have been\n"
     "created by carving out a survey footprint from simulated data. The module can also be used "
     ...
     ">>> nmu_bins=10\n"
     ">>> mu_max=1.0\n"
     ">>> (DDsmu, time) = countpairs_s_mu_mocks(autocorr, cosmology, nthreads, mu_max, nmu_bins, "
     "binfile,\n"
     "                                            ra,dec,cz,ra,dec,cz,\n"
     "                                            verbose=True)\n"
     "\n"},
    {"countpairs_theta_mocks", (PyCFunction)countpairs_countpairs_theta_mocks,
     METH_VARARGS | METH_KEYWORDS,
     ...
     "\n"
     "Calculate the angular pair-counts, required for " OMEGA_CHAR "(" THETA_CHAR
     "), auto/cross-correlation function given two\n"
     "sets of RA1/DEC1 and RA2/DEC2 arrays. This module is suitable for mock catalogs that have "
     "been\n"
     "created by carving out a survey footprint from simulated data. The module can also be used "
     "for actual\n"
     "observed galaxies, but you probably want to attach weights to the points to account for "
     ...
     "\n"
     "Note, that this module only returns pair counts and not the actual correlation function\n"
     "" OMEGA_CHAR "(" THETA_CHAR
     "). See  `~Corrfunc.utils.convert_3d_counts_to_cf.py` for computing\n"
     ...
     "   ``thetaravg`` will be set to 0.0 for all bins; similarly for ``weightavg``. ``npairs`` "
     ...
     "                              verbose=True)\n"
     "\n"},
    {"countspheres_vpf_mocks", (PyCFunction)countpairs_countspheres_vpf_mocks,
     METH_VARARGS | METH_KEYWORDS,
...
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT,       "_countpairs_mocks",     module_docstring,
    sizeof(struct module_state), module_methods,          NULL,
    _countpairs_mocks_traverse,  _countpairs_mocks_clear, NULL};
...
static int print_kwlist_into_msg(char *msg, const size_t totsize, size_t len, char *kwlist[],
                                 const size_t nitems)
{
...
static int64_t check_dims_and_datatype(PyObject *module, PyArrayObject *x1_obj,
                                       PyArrayObject *y1_obj, PyArrayObject *z1_obj,
                                       PyArrayObject *weights1_obj, size_t *element_size)
{
  char msg[1024];
  ...
  if (n_weight_dims != 1 && n_weight_dims != 2) {
    snprintf(msg, 1024,
             "ERROR: Expected 1-D or 2-D weight array.\nFound n_weight_dims = %d instead",
             n_weight_dims);
    countpairs_mocks_error_out(module, msg);
    return -1;
  }
  ...
  if (!((x_type == NPY_FLOAT || x_type == NPY_DOUBLE) &&
    ...
    if (x_descr == NULL || y_descr == NULL || z_descr == NULL || weights_descr == NULL) {
      ...
      snprintf(msg, 1024,
               "TypeError: Expected floating point arrays (allowed types = %d or %d). Instead "
               "found type-nums (%d, %d, %d, %d)\n",
      ...
      snprintf(msg, 1024,
               ...
               NPY_FLOAT, NPY_DOUBLE, x_type, y_type, z_type, weights_type,
               x_descr->typeobj->tp_name, y_descr->typeobj->tp_name, z_descr->typeobj->tp_name,
               weights_descr->typeobj->tp_name);
    }
    Py_XDECREF(x_descr);
    Py_XDECREF(y_descr);
    Py_XDECREF(z_descr);
    Py_XDECREF(weights_descr);
    countpairs_mocks_error_out(module, msg);
    return -1;
  }
  ...
  if (x_type != y_type || y_type != z_type || (check_weights && z_type != weights_type)) {
    ...
    if (x_descr == NULL || y_descr == NULL || z_descr == NULL || weights_descr == NULL) {
      /* Generating the dtype descriptor failed somehow. At least provide some information */
      snprintf(msg, 1024,
               "TypeError: Expected *ALL* 3 floating point arrays to be the same type (allowed "
               "types = %d or %d). Instead found type-nums (%d, %d, %d, %d)\n",
               NPY_FLOAT, NPY_DOUBLE, x_type, y_type, z_type, weights_type);
    } else {
      snprintf(msg, 1024,
               "TypeError: Expected *ALL* 3 floating point arrays to be the same type (allowed "
  ...
  if (nx1 != ny1 || ny1 != nz1) {
    snprintf(msg, 1024,
             "ERROR: Expected arrays to have the same number of elements in all "
  ...
  if (check_weights) {
    ...
    if (nx1 != n_weights1) {
      snprintf(msg, 1024,
               "ERROR: the last dimension of `weights` must match the number of positions.  "
...
static int64_t check_dims_and_datatype_ra_dec(PyObject *module, PyArrayObject *x1_obj,
                                              PyArrayObject *y1_obj, size_t *element_size)
{
  char msg[1024];
  ...
  if (nxdims != 1 || nydims != 1) {
    snprintf(msg, 1024,
             "ERROR: Expected 1-D numpy arrays.\nFound (nxdims, nydims) = (%d, %d) instead", nxdims,
             nydims);
    countpairs_mocks_error_out(module, msg);
    return -1;
  }
  ...
  if (!((x_type == NPY_FLOAT || x_type == NPY_DOUBLE) &&
    ...
    if (x_descr == NULL || y_descr == NULL) {
      /* Generating the dtype descriptor failed somehow. At least provide some information */
      snprintf(msg, 1024,
               "TypeError: Expected 2 floating point arrays (allowed types = %d or %d). Instead "
               "found type-nums (%d, %d)\n",
      ...
      snprintf(msg, 1024,
               ...
               "found type-nums (%d, %d) "
               "with type-names = (%s, %s)\n",
               NPY_FLOAT, NPY_DOUBLE, x_type, y_type, x_descr->typeobj->tp_name,
               y_descr->typeobj->tp_name);
    }
    Py_XDECREF(x_descr);
    Py_XDECREF(y_descr);
    countpairs_mocks_error_out(module, msg);
  ...
  if (x_type != y_type) {
    ...
    if (x_descr == NULL || y_descr == NULL) {
      /* Generating the dtype descriptor failed somehow. At least provide some information */
      snprintf(msg, 1024,
               "TypeError: Expected *BOTH* (RA, DEC) floating point arrays to be the same type "
               "(allowed types = %d or %d). Instead found type-nums (%d, %d)\n",
               NPY_FLOAT, NPY_DOUBLE, x_type, y_type);
    } else {
      snprintf(msg, 1024,
               "TypeError: Expected *BOTH* (RA, DEC) floating point arrays to be the same type "
  ...
  if (nx1 != ny1) {
    snprintf(msg, 1024,
             "ERROR: Expected *BOTH* (RA, DEC) arrays to have the same number of elements in all "
             "2-dimensions.\nFound (nra, ndec) = "
             "(%" PRId64 ", %" PRId64 ") instead",
             nx1, ny1);
    countpairs_mocks_error_out(module, msg);
    return -1;
  }
...
{
// Error-handling is global in python2 -> stored in struct module_state _struct declared at the top
// of this file
#if PY_MAJOR_VERSION < 3
  (void)self;
  PyObject *module = NULL;    // should not be used -> setting to NULL so any attempts to
                              // dereference will result in a crash.
#else
  ...
  int8_t xbin_ref = options.bin_refine_factors[0], ybin_ref = options.bin_refine_factors[1],
  ...
  static char
      *kwlist[] = {"autocorr",
                   ...
                   "max_cells_per_dim",
                   "c_api_timer",
                   "isa", /* instruction set to use of type enum isa; valid values are AVX, SSE,
                             FALLBACK (enum) */
                   "weight_type",
                   NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "iiidsO!O!O!|O!O!O!O!O!bbbbbbbhbis", kwlist, &autocorr, &cosmology,
          &nthreads, &pimax, &binfile, &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj,
          &PyArray_Type, &z1_obj, &PyArray_Type, &weights1_obj, &PyArray_Type,
          &x2_obj,    // optional parameters -> if autocorr == 1, not checked; required if
                      // autocorr=0
          &PyArray_Type, &y2_obj, &PyArray_Type, &z2_obj, &PyArray_Type, &weights2_obj,
          &(options.is_comoving_dist), &(options.verbose), &(options.need_avg_sep),
          &(options.fast_divide), &xbin_ref, &ybin_ref, &zbin_ref, &(options.max_cells_per_dim),
          &(options.c_api_timer), &(options.instruction_set), &weighting_method_str)
    ...
    int len = snprintf(
        msg, 1024,
        "ArgumentError: In DDrppi_mocks> Could not parse the arguments. Input parameters are: \n");
  ...
  if (xbin_ref != options.bin_refine_factors[0] || ybin_ref != options.bin_refine_factors[1] ||
      zbin_ref != options.bin_refine_factors[2]) {
    options.bin_refine_factors[0] = xbin_ref;
    options.bin_refine_factors[1] = ybin_ref;
    options.bin_refine_factors[2] = zbin_ref;
    set_bin_refine_scheme(
        &options, BINNING_CUST);    // custom binning -> code will honor requested binning scheme
  }
  ...
  size_t element_size;
  const int64_t ND1 =
      check_dims_and_datatype(module, x1_obj, y1_obj, z1_obj, weights1_obj, &element_size);
  if (ND1 == -1) {
  ...
  if (wstatus != EXIT_SUCCESS) {
    char msg[1024];
    snprintf(msg, 1024, "ValueError: In %s: unknown weight_type %s!", __FUNCTION__,
  ...
  if (extra.weights0.num_weights > 0 && extra.weights0.num_weights != found_weights) {
    char msg[1024];
    snprintf(msg, 1024,
             "ValueError: In %s: specified weighting method %s which requires %" PRId64
             " weight(s)-per-particle, but found %d weight(s) instead!\n",
  ...
  if (extra.weights0.num_weights > 0 && found_weights > MAX_NUM_WEIGHTS) {
    char msg[1024];
    snprintf(msg, 1024,
             "ValueError: In %s: Provided %d weights-per-particle, but the code was compiled with "
             "MAX_NUM_WEIGHTS=%d.\n",
  ...
  if (autocorr == 0) {
    ...
    if (x2_obj == NULL || y2_obj == NULL || z2_obj == NULL) {
      snprintf(msg, 1024,
               "ValueError: In %s: If autocorr is 0, need to pass the second set of positions "
    ...
    if ((weights1_obj == NULL) != (weights2_obj == NULL)) {
      snprintf(
          msg, 1024,
          "ValueError: In %s: If autocorr is 0, must pass either zero or two sets of weights.\n",
          __FUNCTION__);
      countpairs_mocks_error_out(module, msg);
      Py_RETURN_NONE;
    }
    ...
    if (element_size != element_size2) {
      snprintf(msg, 1024,
               "TypeError: In %s: The two arrays must have the same data-type. First array is of "
               "type %s while second array is of type %s\n",
               __FUNCTION__, element_size == 4 ? "floats" : "doubles",
               element_size2 == 4 ? "floats" : "doubles");
      countpairs_mocks_error_out(module, msg);
      Py_RETURN_NONE;
    }
  }
  ...
  if (x1_array == NULL || y1_array == NULL || z1_array == NULL ||
    ...
    char msg[1024];
    snprintf(msg, 1024,
             "TypeError: In %s: Could not convert input to arrays of allowed floating point types "
             "(doubles or floats). Are you passing numpy arrays?",
             __FUNCTION__);
    countpairs_mocks_error_out(module, msg);
    Py_RETURN_NONE;
  }
  ...
  int status = countpairs_mocks(ND1, phiD1, thetaD1, czD1, ND2, phiD2, thetaD2, czD2, nthreads,
                                autocorr, binfile, pimax, cosmology, &results, &options, &extra);
  ...
  Py_DECREF(y1_array);
  Py_DECREF(z1_array);
  Py_XDECREF(weights1_array);    // x1 should absolutely not be NULL
  Py_XDECREF(x2_array);
  Py_XDECREF(y2_array);
  Py_XDECREF(z2_array);
  Py_XDECREF(weights2_array);    // x2 might be NULL depending on value of autocorr
  ...
  for (int i = 1; i < results.nbin; i++) {
    for (int j = 0; j < results.npibin; j++) {
      ...
      item = Py_BuildValue("(ddddkd)", rlow, results.rupp[i], rpavg, (j + 1) * dpi,
                           results.npairs[bin_index], weight_avg);
...
// of this file
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "iiidisO!O!O!|O!O!O!O!O!bbbbbbbhbis", kwlist, &autocorr, &cosmology,
          &nthreads, &mu_max, &nmu_bins, &binfile, &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj,
          &PyArray_Type, &z1_obj, &PyArray_Type, &weights1_obj, &PyArray_Type,
          &x2_obj,    // optional parameters -> if autocorr == 1, not checked; required if
                      // autocorr=0
          &PyArray_Type, &y2_obj, &PyArray_Type, &z2_obj, &PyArray_Type, &weights2_obj,
    ...
    int len = snprintf(
        msg, 1024,
        "ArgumentError: In DDsmu_mocks> Could not parse the arguments. Input parameters are: \n");
  ...
  double c_api_time = 0.0;
  int status = countpairs_mocks_s_mu(ND1, phiD1, thetaD1, czD1, ND2, phiD2, thetaD2, czD2, nthreads,
                                     autocorr, binfile, mu_max, nmu_bins, cosmology, &results,
                                     &options, &extra);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  ...
  for (int i = 1; i < results.nsbin; i++) {
    for (int j = 0; j < results.nmu_bins; j++) {
      ...
      const double weight_avg = results.weightavg[bin_index];
      item = Py_BuildValue("(ddddkd)", rlow, results.supp[i], savg, (j + 1) * dmu,
                           results.npairs[bin_index], weight_avg);
      PyList_Append(ret, item);
      Py_XDECREF(item);
    }
...
// of this file
  ...
  options.fast_acos = 0;
  options.c_api_timer = 0;
  int8_t ra_bin_ref = options.bin_refine_factors[0], dec_bin_ref = options.bin_refine_factors[1];
  static char *kwlist[] = {
      "autocorr",
      "nthreads",
      "binfile",
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "iisO!O!|O!O!O!O!bbbbbbbhbis", kwlist, &autocorr, &nthreads, &binfile,
          &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj, &PyArray_Type, &weights1_obj,
          &PyArray_Type, &x2_obj,    // optional parameters -> if autocorr == 1, not checked;
                                     // required if autocorr=0
          &PyArray_Type, &y2_obj, &PyArray_Type, &weights2_obj, &(options.link_in_dec),
          &(options.link_in_ra), &(options.verbose), &(options.need_avg_sep), &(options.fast_acos),
          &ra_bin_ref, &dec_bin_ref, &(options.max_cells_per_dim), &(options.c_api_timer),
          &(options.instruction_set), &weighting_method_str)
    ...
    int len = snprintf(
        msg, 1024,
        "ArgumentError: In DDtheta_mocks> Could not parse the arguments. Input parameters are: \n");
  ...
  if (ra_bin_ref != options.bin_refine_factors[0] || dec_bin_ref != options.bin_refine_factors[1]) {
  ...
  if (autocorr == 0) {
    ...
    if (x2_obj == NULL || y2_obj == NULL) {
      snprintf(msg, 1024,
               "ValueError: In %s: If autocorr is 0, need to pass the second set of positions "
               "(RA2=numpy array, DEC2=numpy array).\n",
               __FUNCTION__);
      countpairs_mocks_error_out(module, msg);
      Py_RETURN_NONE;
    }
  ...
  options.float_type = element_size;
  double c_api_time = 0.0;
  int status = countpairs_theta_mocks(ND1, phiD1, thetaD1, ND2, phiD2, thetaD2, nthreads, autocorr,
                                      binfile, &results, &options, &extra);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  NPY_END_THREADS;
  ...
  Py_DECREF(x1_array);
  Py_DECREF(y1_array);
  Py_XDECREF(weights1_array);    // x1/y1 (representing ra1,dec1) should not be NULL
  Py_XDECREF(x2_array);
  Py_XDECREF(y2_array);
  Py_XDECREF(weights2_array);    // x2/y2 may be NULL (in case of autocorr)
  ...
  for (int i = 1; i < results.nbin; i++) {
    ...
    item = Py_BuildValue("(dddkd)", rlow, results.theta_upp[i], theta_avg, results.npairs[i],
                         weight_avg);
    PyList_Append(ret, item);
    Py_XDECREF(item);
...
// of this file
  ...
  // x1->ra (phi), y1-> declination (theta1), z1->cz (cz1)
  // x2->ra (ph2), y2-> declination (theta2), z2->cz (cz2)
  PyArrayObject *x1_obj = NULL, *y1_obj = NULL, *z1_obj = NULL, *x2_obj = NULL, *y2_obj = NULL,
                *z2_obj = NULL;
  int cosmology = 1;
  double rmax;
  ...
  /* Reset the bin refine factors default (since the VPF is symmetric in XYZ, conceptually the
   * binning should be identical in all three directions)*/
  int bin_ref[] = {1, 1, 1};
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "diiiisiO!O!O!O!O!O!|bbbbbhbi", kwlist, &rmax, &nbin, &num_spheres, &num_pN,
          &threshold_neighbors, &centers_file, &cosmology, &PyArray_Type, &x1_obj, &PyArray_Type,
          &y1_obj, &PyArray_Type, &z1_obj, &PyArray_Type, &x2_obj, &PyArray_Type, &y2_obj,
          &PyArray_Type, &z2_obj, &(options.is_comoving_dist), &(options.verbose), &xbin_ref,
          &ybin_ref, &zbin_ref, &(options.max_cells_per_dim), &(options.c_api_timer),
    ...
    int len = snprintf(
        msg, 1024,
        "ArgumentError: In vpf_mocks> Could not parse the arguments. Input parameters are: \n");
  ...
  if (x1_array == NULL || y1_array == NULL || z1_array == NULL || x2_array == NULL ||
      y2_array == NULL || z2_array == NULL) {
    Py_XDECREF(x1_array);
    Py_XDECREF(y1_array);
    Py_XDECREF(z1_array);
  ...
  options.float_type = element_size;
  double c_api_time = 0.0;
  int status = countspheres_mocks(ND1, phiD1, thetaD1, czD1, ND2, phiD2, thetaD2, czD2,
                                  threshold_neighbors, rmax, nbin, num_spheres, num_pN,
                                  centers_file, cosmology, &results, &options, NULL);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  NPY_END_THREADS;
