...
} isa;           // name for instruction sets -> corresponds to the return from instrset_detect in
          // cpu_features.c
...
#define BINNING_REF_MASK                                                                           \
  0x0000000F    // Last 4 bits for how the bin sizes are calculated is done. Also indicates if
...
struct api_cell_timings {
  ...
  int tid; /* Thread-id, 0 for serial case, wastes 4 bytes, since thread id is 4bytes integer and
              not 8 bytes */
};
...
struct config_options {
  ...
  size_t float_type;       /* floating point type -> vectorized supports double/float; fallback can
                              support long double*/
  ...
  int8_t bin_refine_factors[3]; /* Array for the custom bin refine factors in each dim
                                  ...
                                  Must be signed integers since some for loops might use
                                  -bin_refine_factor as the starting point */

  uint16_t
      max_cells_per_dim; /* max number of cells per dimension. same for both theory and mocks */
  union {
  ...
  uint8_t reserved[OPTIONS_HEADER_SIZE - 33 * sizeof(char) - sizeof(size_t) - 9 * sizeof(double) -
                   3 * sizeof(int) - sizeof(uint16_t) - 14 * sizeof(uint8_t) -
                   sizeof(struct api_cell_timings *) - sizeof(int64_t)];
};
...
static inline void set_bin_refine_factors(struct config_options *options,
                                          const int bin_refine_factors[3])
{
  for (int i = 0; i < 3; i++) {
...
static inline void set_custom_bin_refine_factors(struct config_options *options,
                                                 const int bin_refine_factors[3])
{
...
static inline void set_max_cells(struct config_options *options, const int max)
{
  if (max <= 0) {
    fprintf(stderr,
            "Warning: Max. cells per dimension was requested to be set to "
  ...
  if (max > INT16_MAX) {
    fprintf(stderr,
            "Warning: Max cells per dimension is a 2-byte integer and can not "
...
static inline struct config_options get_config_options(void)
{
  ENSURE_STRUCT_SIZE(
      struct config_options,
      OPTIONS_HEADER_SIZE);    // compile-time check for making sure struct is correct size

  if (strncmp(API_VERSION, STR(VERSION), 32) != 0) {
    fprintf(stderr,
            "Error: Version mismatch between header and Makefile. Header claims version = `%s' "
            "while Makefile claims version = `%s'\n"
            "Library header probably needs to be updated\n",
            API_VERSION, STR(VERSION));
    exit(EXIT_FAILURE);
  }
  struct config_options options;
  BUILD_BUG_OR_ZERO(sizeof(options.max_cells_per_dim) == sizeof(int16_t),
                    max_cells_per_dim_must_be_16_bits);
  BUILD_BUG_OR_ZERO(sizeof(options.binning_flags) == sizeof(uint32_t),
                    binning_flags_must_be_32_bits);
  BUILD_BUG_OR_ZERO(sizeof(options.bin_refine_factors[0]) == sizeof(int8_t),
                    bin_refine_factors_must_be_8_bits);
...
struct extra_options {
  ...
  weight_method_t
      weight_method;    // the function that will get called to give the weight of a particle pair
...
static inline struct extra_options get_extra_options(const weight_method_t weight_method)
{
  ...
  ENSURE_STRUCT_SIZE(
      struct extra_options,
      EXTRA_OPTIONS_HEADER_SIZE);    // compile-time check for making sure struct is correct size
...
static inline void print_cell_timings(struct config_options *options)
{
  ...
  for (int64_t i = 0; i < options->totncells_timings; i++) {
    fprintf(stderr, "%8d %8d %12" PRId64 " %12" PRId64 " %12" PRId64 " %12d\n",
            options->cell_timings[i].first_cellindex, options->cell_timings[i].second_cellindex,
            options->cell_timings[i].N1, options->cell_timings[i].N2,
            options->cell_timings[i].time_in_ns, options->cell_timings[i].tid);
  }
}
...
static inline void assign_cell_timer(struct api_cell_timings *cell_timings, const int64_t totncells,
                                     const int max_ngb_cells, struct config_options *options)
{
  ...
  if (options->totncells_timings < totncells_timings) {
    ...
    if (options->cell_timings == NULL) {
      fprintf(stderr,
              "Warning: In %s> Could not allocate memory to store the API timings per cell. \n",
              __FUNCTION__);
    } else {
  ...
  /* This looks like a repeated "if" condition but it is not. Covers the case for the calloc failure
   * above */
  if (options->totncells_timings >= totncells_timings) {
