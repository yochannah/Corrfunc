...
void byte_swap(char *const in, const size_t size, char *out)
{
  if (size > 16) {
    fprintf(stderr,
            "WARNING: In %s> About to byte_swap %zu bytes but no intrinsic C data-type exists with "
            "size larger than 16 bytes",
            __FUNCTION__, size);
  }
...
void *my_realloc(void *x, size_t size, int64_t N, const char *varname)
{
  ...
  if (tmp == NULL) {
    fprintf(stderr,
            "ERROR: Could not reallocate for %" PRId64
            " elements with %zu size for variable `%s' ..aborting\n",
            N, size, varname);
    perror(NULL);
  }
...
/*         fprintf(stderr,"Error: Comparison between floats should have smaller number of max. units
...
int AlmostEqualRelativeAndAbs_float(float A, float B, const float maxDiff, const float maxRelDiff)
{
...
int AlmostEqualRelativeAndAbs_double(double A, double B, const double maxDiff,
