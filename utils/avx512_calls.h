...
#define AVX512_MASK_MULTIPLY_INTS(FALSEVALS, MASK, X, Y)                                           \
  _mm512_mask_mul_epi32(FALSEVALS, MASK, X, Y)
#define AVX512_MASKZ_MULTIPLY_INTS(MASK, X, Y) _mm512_maskz_mul_epi32(MASK, X, Y)
#endif /* commenting out the int math operations since they are either cumbersome or produce       \
          results of different SIMD widths*/

#define AVX512_LOAD_FLOATS_UNALIGNED(X) _mm512_loadu_ps(X)
#define AVX512_MASK_LOAD_FLOATS_UNALIGNED(FALSEVALS, MASK, X)                                      \
  _mm512_mask_loadu_ps(FALSEVALS, MASK, X)
...
#define AVX512_MASK_MULTIPLY_INTS(FALSEVALS, MASK, X, Y)                                           \
  _mm256_mask_mul_epi32(FALSEVALS, MASK, X, Y)
#define AVX512_MASKZ_MULTIPLY_INTS(MASK, X, Y) _mm256_maskz_mul_epi32(MASK, X, Y)

#elif defined(__AVX512F__)
#define AVX512_MASK_ADD_INTS(FALSEVALS, MASK, X, Y)                                                \
  _mm512_castsi512_si256(_mm512_mask_add_epi32(_mm512_castsi256_si512(FALSEVALS), MASK,            \
                                               _mm512_castsi256_si512(X),                          \
                                               _mm512_castsi256_si512(Y)))
#define AVX512_MASKZ_ADD_INTS(MASK, X, Y)                                                          \
  _mm512_castsi512_si256(                                                                          \
      _mm512_maskz_add_epi32(MASK, _mm512_castsi256_si512(X), _mm512_castsi256_si512(Y)))

#define AVX512_MASK_MULTIPLY_INTS(FALSEVALS, MASK, X, Y)                                           \
  _mm512_castsi512_si256(_mm512_mask_mul_epi32(_mm512_castsi256_si512(FALSEVALS), MASK,            \
                                               _mm512_castsi256_si512(X),                          \
                                               _mm512_castsi256_si512(Y)))
#define AVX512_MASKZ_MULTIPLY_INTS(MASK, X, Y)                                                     \
  _mm512_castsi512_si256(                                                                          \
      _mm512_maskz_mul_epi32(MASK, _mm512_castsi256_si512(X), _mm512_castsi256_si512(Y)))
#endif
#endif /* commenting out the int math operations since they are either cumbersome or produce       \
          results of different SIMD widths*/
...
#define AVX512_MASK_LOAD_FLOATS_UNALIGNED(FALSEVALS, MASK, X)                                      \
  _mm512_mask_loadu_pd(FALSEVALS, MASK, X)
...
#define AVX512_BLEND_FLOATS_WITH_MASK(MASK, FALSEVALUE, TRUEVALUE)                                 \
  _mm512_mask_blend_pd(MASK, FALSEVALUE, TRUEVALUE)

#if defined(__AVX512VL__)
#define AVX512_BLEND_INTS_WITH_MASK(MASK, FALSE, TRUE) _mm256_mask_blend_epi32(MASK, FALSE, TRUE)
#elif defined(__AVX2__)
#define AVX512_BLEND_INTS_WITH_MASK(MASK, FALSE, TRUE)                                             \
  _mm256_blend_epi32(FALSE, TRUE, MASK)    // AVX2
#else
...
static inline AVX512_FLOATS inv_cosine_avx512(const AVX512_FLOATS X, const int order)
{
  ...
  if (order == 0) {
    ...
    // fast acos
    /*Taken from associated C++ code in
     * http://www.geometrictools.com/GTEngine/Include/Mathematics/GteACosEstimate.h*/
    for (int ii = 0; ii < AVX512_NVEC; ii++) {
