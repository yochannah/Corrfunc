...
/* The max. value of bin refine factor to probe. Each of bin refinements factors is set from [1,
 max_binref] (inclusive) */
          ...
          for (int bfz = min_bin_ref; bfz <= max_bin_ref; bfz++) {                                 \
            if (dotest == 1) {                                                                     \
              ...
              fprintf(stderr, "Running with bin refs = (%d, %d, %d) and instruction set = %s...",  \
                      ...
                      options.bin_refine_factors[2], isa_name[iset]);                              \
              current_utc_time(&t0);
...
#define END_INTEGRATION_TEST_SECTION                                                               \
  ...
  if (ret == EXIT_SUCCESS) {                                                                       \
    fprintf(stderr,                                                                                \
            ANSI_COLOR_MAGENTA                                                                     \
            "Fastest time = %8.2lf seconds with bin-ref = {%d, %d, %d}" ANSI_COLOR_RESET "\n",     \
            fastest_time * 1e-9, fastest_bin_ref[0], fastest_bin_ref[1], fastest_bin_ref[2]);      \
  }                                                                                                \
  } /*instruction set */                                                                           \
