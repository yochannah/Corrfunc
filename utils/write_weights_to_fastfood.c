...
int main(int argc, char **argv)
{
  ...
  if (argc != 3) {
    fprintf(stderr,
            "Error: Usage `%s' <text file with weights> <fastfood file with positions/velocities "
            "to append to> \n",
            argv[0]);
    return EXIT_FAILURE;
  }
  ...
  if (nread != 1 || dummy != 20) {
    ...
    perror(NULL);
    fprintf(stderr,
            "Error: Incorrect format for fast-food fortran binary. Expected to read bytes showing "
            "data for a header of 5 integers\n"
  ...
  if (nread != 5 || idat[1] <= 0) {
    ...
    perror(NULL);
    fprintf(stderr,
            "Error: Expected to read header with 5 integers. Read = %zu items with second element "
            "= %d as the number of galaxies contained (should be positive).\n",
            nread, idat[1]);
    return EXIT_FAILURE;
  }
  ...
  if (!(nread == 1 && (dummy == 4 * Ngal || dummy == 8 * Ngal))) {
    ...
    perror(NULL);
    fprintf(stderr,
            "Error: Expected to read one item of padding bytes, read = %zu items. Positions can be "
            "either float (4 bytes) or doubles (8 bytes) - found %d instead.\n"
