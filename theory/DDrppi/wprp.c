...
int main(int argc, char *argv[])
{
  ...
  i = 0;
  while (fscanf(fpD1D2, "%lf %lf %lf %lf%*[^\n]", &logrp1[i], &pi1[i], &ravg1[i], &D1D2[i]) !=
         EOF) {
    /* fprintf(stderr,"%lf %lf %lf %lf\n",D1D2[i],ravg1[i],logrp1[i],pi1[i]); */
    i++;
    XASSERT(i <= ndat,
            "Current line parsed = %d must be less than number of data points expected = %d\n"
            "nrpbin = %d npibin = %d",
            i, ndat, nrpbin, npibin);
  }
  ...
  for (i = 0; i < nrpbin; i++) {
    ...
    fprintf(stdout, "%12.6lf %12.6lf %12.6lf %12.6lf %12" PRId64 "\n", rpmin, rpmax, rpavg[i],
            wp[i], (int64_t)DDtot[i]);    // rpavg actually contains the upper limit of the bin
    rpmin = rpmax;
  }
