...
int countspheres(const int64_t np, void *restrict X, void *restrict Y, void *restrict Z,
                 const double rmax, const int nbin, const int nc, const int num_pN,
                 unsigned long seed, results_countspheres *results, struct config_options *options,
                 struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countspheres_float(np, (float *)X, (float *)Y, (float *)Z, rmax, nbin, nc, num_pN, seed,
                              results, options, extra);
  } else {
    return countspheres_double(np, (double *)X, (double *)Y, (double *)Z, rmax, nbin, nc, num_pN,
                               seed, results, options, extra);
  }
}
