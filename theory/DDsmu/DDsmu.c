...
--- DDsmu file1 format1 file2 format2 binfile mu_max nmu_bins numthreads [weight_method
weights_file1 weights_format1 [weights_file2 weights_format2]] > DDfile
 ...
 * mu_max        = maximum of the cosine of the angle to the line-of-sight (LOS is taken to be along
the z-direction)
 * nmu_bins      = number of bins for mu
 * numthreads    = number of threads to use
--- OPTIONAL ARGS:
 * weight_method = the type of pair weighting to apply.  Options are: 'pair_product', 'none'.
Default: 'none'.
...
int main(int argc, char *argv[])
{
  ...
  char *fileformat1 = NULL, *fileformat2 = NULL, *weights_fileformat1 = NULL,
       *weights_fileformat2 = NULL;
  char *sbinfile = NULL;
  double mu_max;
  int nmu_bins;
  ...
  DOUBLE *x2 = NULL, *y2 = NULL, *z2 = NULL,
         *weights2[MAX_NUM_WEIGHTS] = {NULL};    // will point to x1/y1/z1 in case of auto-corr

  int nthreads = 1;
  /*---Corrfunc-variables----------------*/
#if !(defined(USE_OMP) && defined(_OPENMP))
  const char argnames[][30] = {"file1",    "format1", "file2",   "format2",
                               "sbinfile", "mu_max",  "nmu_bins"};
#else
  const char argnames[][30] = {"file1",    "format1", "file2",    "format2",
                               "sbinfile", "mu_max",  "nmu_bins", "Nthreads"};
#endif
  const char optargnames[][30] = {"weight_method", "weights_file1", "weights_format1",
                                  "weights_file2", "weights_format2"};
  ...
  if (noptargs_given != 0 && noptargs_given != 3 && noptargs_given != 5) {
    Printhelp();
    fprintf(stderr,
            "\nFound: %d optional arguments; must be 0 (no weights), 3 (for one set of weights) or "
            "5 (for two sets)\n ",
            noptargs_given);
    int i;
  ...
  if (weights_file1 != NULL) {
    gettimeofday(&t0, NULL);
    int64_t wND1 = read_columns_into_array(weights_file1, weights_fileformat1, sizeof(DOUBLE),
                                           num_weights, (void **)weights1);
    gettimeofday(&t1, NULL);
    read_time += ADD_DIFF_TIME(t0, t1);

    if (wND1 != ND1) {
      fprintf(stderr, "Error: read %" PRId64 " lines from %s, but read %" PRId64 " from %s\n", wND1,
              weights_file1, ND1, file1);
      return EXIT_FAILURE;
    }
  }
  ...
  if (autocorr == 0) {
    ...
    if (weights_file2 != NULL) {
      gettimeofday(&t0, NULL);
      int64_t wND2 = read_columns_into_array(weights_file2, weights_fileformat2, sizeof(DOUBLE),
                                             num_weights, (void **)weights2);
      gettimeofday(&t1, NULL);
      read_time += ADD_DIFF_TIME(t0, t1);
  ...
  int status = countpairs_s_mu(ND1, x1, y1, z1, ND2, x2, y2, z2, nthreads, autocorr, sbinfile,
  ...
  for (int i = 1; i < results.nsbin; i++) {
    ...
    for (int j = 0; j < nmu_bins; j++) {
      ...
      fprintf(stdout, "%e\t%e\t%e\t%12" PRIu64 "\t%e\n", smin, smax, (j + 1) * dmu,
              results.npairs[index], results.weightavg[index]);
    }
    smin = smax;
  }
  ...
  fprintf(stderr,
          "DDsmu> Done -  ND1=%12" PRId64 " ND2=%12" PRId64 ". Time taken = %6.2lf seconds. "
                                                            "read-in time = %6.2lf seconds "
                                                            "pair-counting time = %6.2lf sec\n",
...
void Printhelp(void)
{
  fprintf(stderr, "=========================================================================\n");
#if defined(USE_OMP) && defined(_OPENMP)
  fprintf(stderr, "   --- DDsmu file1 format1 file2 format2 sbinfile mu_max nmu_bins numthreads "
                  "[weight_method weights_file1 weights_format1 [weights_file2 weights_format2]] > "
                  "DDfile\n");
#else
  fprintf(stderr, "   --- DDsmu file1 format1 file2 format2 sbinfile mu_max nmu_bins "
                  "[weight_method weights_file1 weights_format1 [weights_file2 weights_format2]] > "
                  "DDfile\n");
#endif
  ...
  fprintf(
      stderr,
      "     * sbinfile      = name of ascii file containing the s-bins (smin smax for each bin)\n");
  fprintf(stderr, "     * mu_max        = maximum of the cosine of the angle to the line-of-sight "
                  "(LOS is taken to be along the z-direction). Valid values are in: (0.0, 1.0]\n");
  fprintf(stderr, "     * nmu_bins      = number of bins for mu (must be >= 1)\n");
#if defined(USE_OMP) && defined(_OPENMP)
  fprintf(stderr, "     * numthreads    = number of threads to use (must be >= 1)\n");
#endif
  fprintf(stderr, "   --- OPTIONAL ARGS:\n");
  fprintf(stderr, "     * weight_method = the type of pair weighting to apply.  Options are: "
                  "'pair_product', 'none'.  Default: 'none'.\n");
  fprintf(stderr, "     * weights_file1 = name of file containing the weights corresponding to the "
                  "first data file\n");
  fprintf(stderr, "     * weights_format1 = format of file containing the weights corresponding to "
                  "the first data file\n");
  fprintf(stderr, "     * weights_file2 = name of file containing the weights corresponding to the "
                  "second data file\n");
  fprintf(stderr, "     * weights_format2 = format of file containing the weights corresponding to "
                  "the second data file\n");
  fprintf(stderr, "   ---OUTPUT:\n");
#ifdef OUTPUT_RPAVG
  fprintf(stderr, "     > DD(s, mu) file        = name of output file <smin smax mu_max_bin savg "
                  "npairs weightavg>\n");
#else
  fprintf(stderr, "     > DD(s, mu) file        = name of output file <smin smax mu_max_bin  0.0 "
                  "npairs weightavg>\n");
#endif
  fprintf(stderr, "\n\tCompile options: \n");
#ifdef PERIODIC
