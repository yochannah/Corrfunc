...
int countpairs_wp(const int64_t ND, void *restrict X, void *restrict Y, void *restrict Z,
                  const double boxsize, const int numthreads, const char *binfile,
                  const double pimax, results_countpairs_wp *results,
                  struct config_options *options, struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countpairs_wp_float(ND, (float *)X, (float *)Y, (float *)Z, boxsize, numthreads, binfile,
                               pimax, results, options, extra);
  } else {
    return countpairs_wp_double(ND, (double *)X, (double *)Y, (double *)Z, boxsize, numthreads,
                                binfile, pimax, results, options, extra);
  }
}
