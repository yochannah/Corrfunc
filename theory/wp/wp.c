...
/* PROGRAM wp
--- wp boxsize file format binfile pimax Nthreads [weight_method weights_file weights_format] >
wpfile
...
int main(int argc, char *argv[])
{
  ...
  if (weights_file != NULL) {
    gettimeofday(&t0, NULL);
    int64_t wND1 = read_columns_into_array(weights_file, weights_fileformat, sizeof(DOUBLE),
  ...
  for (int i = 1; i < results.nbin; ++i) {
    fprintf(stdout, "%e\t%e\t%e\t%12" PRIu64 "\t%e\t%e\n", rlow, results.rupp[i], results.rpavg[i],
            results.npairs[i], results.wp[i], results.weightavg[i]);
    rlow = results.rupp[i];
  }
  ...
  fprintf(stderr,
          "wp> Done -  ND1=%12" PRId64 ". Time taken = %6.2lf seconds. read-in time = %6.2lf "
                                       "seconds pair-counting time = %6.2lf sec\n",
...
void Printhelp(void)
{
  fprintf(stderr, "=========================================================================\n");
#if defined(USE_OMP) && defined(_OPENMP)
  fprintf(stderr, "   --- wp boxsize file format binfile pimax Nthreads [weight_method "
                  "weights_file weights_format] > wpfile\n");
#else
  fprintf(stderr, "   --- wp boxsize file format binfile pimax [weight_method weights_file "
                  "weights_format] > wpfile\n");
#endif
  fprintf(
      stderr,
      "   --- Measure the projected auto-correlation function wp(rp) for a single periodic box\n");
  ...
  fprintf(
      stderr,
      "     * weights_file = name of file containing the weights corresponding to the data file\n");
  fprintf(stderr, "     * weights_format = format of file containing the weights corresponding to "
                  "the data file\n");
  fprintf(stderr, "   ---OUTPUT:\n");
#ifdef OUTPUT_RPAVG
  fprintf(stderr, "     > wpfile        = name of output file. Contains <rmin rmax rpavg npairs wp "
                  "weightavg>\n");
#else
  fprintf(stderr, "     > wpfile        = name of output file. Contains <rmin rmax rpavg=0.0 "
                  "npairs wp weightavg>\n");
#endif
