...
--- DD file1 format1 file2 format2 binfile numthreads [weight_method weights_file1 weights_format1
[weights_file2 weights_format2]] > DDfile
...
int main(int argc, char *argv[])
{
  ...
  int status = countpairs(ND1, x1, y1, z1, ND2, x2, y2, z2, nthreads, autocorr, binfile, &results,
                          &options, &extra); /* This is for ABI compatibility */
  ...
  for (int i = 1; i < results.nbin; i++) {
    fprintf(stdout, "%e\t%e\t%e\t%12" PRIu64 "\t%e\n", rlow, results.rupp[i], results.rpavg[i],
            results.npairs[i], results.weightavg[i]);
    rlow = results.rupp[i];
  }
  ...
  fprintf(stderr,
          "DD> Done -  ND1=%" PRId64 " ND2=%" PRId64 ". Time taken = %6.2lf seconds. read-in time "
                                                     "= %6.2lf seconds sec pair-counting time = "
                                                     "%6.2lf sec\n",
...
void Printhelp(void)
{
  fprintf(stderr, "=========================================================================\n");
#if defined(USE_OMP) && defined(_OPENMP)
  fprintf(stderr, "   --- DD file1 format1 file2 format2 binfile numthreads [weight_method "
                  "weights_file1 weights_format1 [weights_file2 weights_format2]] > DDfile\n");
#else
  fprintf(stderr, "   --- DD file1 format1 file2 format2 binfile [weight_method weights_file1 "
...
#ifdef OUTPUT_RPAVG
  fprintf(stderr,
          "     > DDfile        = name of output file <rmin rmax rpavg npairs weightavg>\n");
#else
  fprintf(stderr,
          "     > DDfile        = name of output file <rmin rmax rpavg=0.0 npairs weightavg>\n");
#endif
