...
int countpairs(const int64_t ND1, void *restrict X1, void *restrict Y1, void *restrict Z1,
               ...
               const int numthreads, const int autocorr, const char *binfile,
               results_countpairs *results, struct config_options *options,
               struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countpairs_float(ND1, (float *)X1, (float *)Y1, (float *)Z1, ND2, (float *)X2,
                            (float *)Y2, (float *)Z2, numthreads, autocorr, binfile, results,
                            options, extra);
  } else {
    return countpairs_double(ND1, (double *)X1, (double *)Y1, (double *)Z1, ND2, (double *)X2,
                             (double *)Y2, (double *)Z2, numthreads, autocorr, binfile, results,
                             options, extra);
  }
}
