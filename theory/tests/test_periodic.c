...
int test_periodic_DD(const char *correct_outputfile)
{
  ...
  for (int i = 1; i < results.nbin; i++) {
    ...
    int floats_equal =
        AlmostEqualRelativeAndAbs_double(rpavg, results.rpavg[i], maxdiff, maxreldiff);
    int weights_equal =
        AlmostEqualRelativeAndAbs_double(weightavg, results.weightavg[i], maxdiff, maxreldiff);
  ...
  if (ret != EXIT_SUCCESS) {
    ...
    for (int i = 1; i < results.nbin; i++) {
      fprintf(fp, "%10" PRIu64 " %20.8lf %20.8lf %20.8lf %20.8lf \n", results.npairs[i],
              results.rpavg[i], rlow, results.rupp[i], results.weightavg[i]);
      rlow = results.rupp[i];
    }
    fclose(fp);
  }
...
int test_periodic_DDsmu(const char *correct_outputfile)
{
  ...
  int status = countpairs_s_mu(ND1, X1, Y1, Z1, ND2, X2, Y2, Z2, nthreads, autocorr, binfile,
                               theory_mu_max, nmu_bins, &results, &options, &extra);
  if (status != EXIT_SUCCESS) {
    return status;
  }
  ...
  if (ret != EXIT_SUCCESS) {
    ...
    for (int i = 1; i < results.nsbin; i++) {
      ...
      for (int j = 0; j < nmubin; j++) {
        int index = i * (nmubin + 1) + j;
        fprintf(fp, "%10" PRIu64 " %20.8lf %20.8lf %20.8lf %20.8lf\n", results.npairs[index],
                results.savg[index], logs, (j + 1) * dmu, results.weightavg[index]);
      }
    }
    fclose(fp);
  }
...
int test_wp(const char *correct_outputfile)
{
  ...
  for (int i = 1; i < results.nbin; i++) {
    ...
    int nitems = fscanf(fp, "%lf %lf %*f %*f %" SCNu64 " %lf%*[^\n]", &wp, &rpavg, &npairs,
                        &weightavg);    // discard rlow and rupp
    if (nitems != 4) {
      ret = EXIT_FAILURE;    // not required but showing intent
      break;
    }
  ...
  if (ret != EXIT_SUCCESS) {
    ...
    double rlow = results.rupp[0];
    for (int i = 1; i < results.nbin; ++i) {
      fprintf(fp, "%e\t%e\t%e\t%e\t%12" PRIu64 "\t%e \n", results.wp[i], results.rpavg[i], rlow,
              results.rupp[i], results.npairs[i], results.weightavg[i]);
      rlow = results.rupp[i];
    }
    fclose(fp);
  }
...
int test_xi(const char *correct_outputfile)
{
  ...
  if (ret != EXIT_SUCCESS) {
    ...
    double rlow = results.rupp[0];
    for (int i = 1; i < results.nbin; ++i) {
      fprintf(fp, "%e\t%e\t%e\t%e\t%12" PRIu64 "\t%e \n", results.xi[i], results.ravg[i], rlow,
              results.rupp[i], results.npairs[i], results.weightavg[i]);
      rlow = results.rupp[i];
    }
    fclose(fp);
...
int main(int argc, char **argv)
{
  ...
  const char alltests_names[][MAXLEN] = {
      "Mr19 DDrppi (periodic)",     "Mr19 DD (periodic)",         "Mr19 wp (periodic)",
      "Mr19 vpf (periodic)",        "Mr19 xi periodic)",          "Mr19 DDsmu (periodic)",
  ...
  const int function_pointer_index[] = {
      1, 0, 2, 3, 4, 5, 1, 1, 1};    //0->DD, 1->DDrppi,2->wp, 3->vpf, 4->xi, 5->DDsmu

  const char correct_outputfiles[][MAXLEN] = {
      "Mr19_DDrppi_periodic", "Mr19_DD_periodic",  "Mr19_wp",
      "Mr19_vpf_periodic",    "Mr19_xi",           "Mr19_DDsmu_periodic",
      "cmass_DD_periodic",    "cmass_DR_periodic", "cmass_RR_periodic"};
  const char firstfilename[][MAXLEN] = {
      "../tests/data/gals_Mr19.ff",        "../tests/data/gals_Mr19.ff",
      "../tests/data/gals_Mr19.ff",        "../tests/data/gals_Mr19.ff",
      "../tests/data/gals_Mr19.ff",        "../tests/data/gals_Mr19.ff",
      "../tests/data/cmassmock_Zspace.ff", "../tests/data/cmassmock_Zspace.ff",
      "../tests/data/random_Zspace.ff"};
  const char firstfiletype[][MAXLEN] = {"f", "f", "f", "f", "f", "f", "f", "f", "f"};
  const char secondfilename[][MAXLEN] = {
      "../tests/data/gals_Mr19.ff",        "../tests/data/gals_Mr19.ff",
      "../tests/data/gals_Mr19.ff",        "../tests/data/gals_Mr19.ff",
      "../tests/data/gals_Mr19.ff",        "../tests/data/gals_Mr19.ff",
      "../tests/data/cmassmock_Zspace.ff", "../tests/data/random_Zspace.ff",
      "../tests/data/random_Zspace.ff"};
