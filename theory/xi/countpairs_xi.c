...
int countpairs_xi(const int64_t ND, void *restrict X, void *restrict Y, void *restrict Z,
                  ...
                  results_countpairs_xi *results, struct config_options *options,
                  struct extra_options *extra)
{
  ...
  if (options->float_type == sizeof(float)) {
    return countpairs_xi_float(ND, (float *)X, (float *)Y, (float *)Z, boxsize, numthreads, binfile,
                               results, options, extra);
  } else {
    return countpairs_xi_double(ND, (double *)X, (double *)Y, (double *)Z, boxsize, numthreads,
