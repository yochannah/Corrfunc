...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < results.nbin; ++i) {
    fprintf(stdout, "%e\t%e\t%e\t%12" PRIu64 "\t%e\t%e\n", rlow, results.rupp[i], results.ravg[i],
            results.npairs[i], results.xi[i], results.weightavg[i]);
    rlow = results.rupp[i];
  }
  ...
  fprintf(stderr,
          "xi> Done -  ND1=%12" PRId64 ". Time taken = %6.2lf seconds. read-in time = %6.2lf "
...
void Printhelp(void)
{
  fprintf(stderr, "=========================================================================\n");
#if defined(USE_OMP) && defined(_OPENMP)
  fprintf(stderr, "   --- xi boxsize file format binfile Nthreads [weight_method weights_file "
                  "weights_format] > xifile\n");
#else
  fprintf(stderr, "   --- xi boxsize file format binfile [weight_method weights_file "
                  "weights_format] > xifile\n");
#endif
...
#ifdef OUTPUT_RPAVG
  fprintf(stderr, "     > xifile        = name of output file. Contains <rmin rmax rpavg npairs xi "
                  "weightavg>\n");
#else
  fprintf(stderr, "     > xifile        = name of output file. Contains <rmin rmax rpavg=0.0 "
                  "npairs xi weightavg>\n");
#endif
