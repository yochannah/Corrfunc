...
static char module_docstring[] =
    "Python extensions for calculating clustering statistics on simulations.\n"
    "\n"
    "countpairs       : Calculate the 3-D xi auto/cross-correlation function given two sets of "
    "arrays with Cartesian XYZ positions.\n"
    "countpairs_rp_pi : Calculate the 2-D DD(" RP_CHAR "," PI_CHAR
    ") auto/cross-correlation function given two sets of arrays with Cartesian XYZ positions.\n"
    "countpairs_wp    : Calculate the projected auto-correlation function wp (assumes PERIODIC) "
    "given one set of arrays with Cartesian XYZ positions\n"
    "countpairs_xi    : Calculate the 3-d auto-correlation function xi (assumes PERIODIC) given "
    "one set of arrays with Cartesian XYZ positions\n"
    "countpairs_s_mu  : Calculate the 2-D DD(s," MU_CHAR
    ") auto/cross-correlation function given two sets of arrays with Cartesian XYZ positions.\n"
    "countpairs_vpf   : Calculate the counts-in-spheres given one set of arrays with Cartesian XYZ "
    "positions\n"
    "\n"
    "See `Corrfunc/call_correlation_functions.py` for example calls to each function in the "
    "extension.\n";
...
static PyMethodDef module_methods[] = {
    /* {"countpairs_error_out"  ,(PyCFunction) countpairs_error_out        ,METH_VARARGS,
       error_out_docstring}, */
    {"countpairs", (PyCFunction)countpairs_countpairs, METH_VARARGS | METH_KEYWORDS,
     "countpairs(autocorr, nthreads, binfile, X1, Y1, Z1, weights1=None, weight_type=None, "
     "periodic=True,\n"
     ...
     "python wrapper for this extension: `Corrfunc.theory.DD` is more "
     "user-friendly.\n" UNICODE_WARNING "\n"
     "Parameters \n"
     "-----------\n"
     ...
     "   will be set to 0.0 for all bins; similarly for ``weight_avg``. ``npairs`` contains the "
     "number of pairs\n"
    ...
    {"countpairs_rp_pi", (PyCFunction)countpairs_countpairs_rp_pi, METH_VARARGS | METH_KEYWORDS,
     "countpairs_rp_pi(autocorr, nthreads, pimax, binfile, X1, Y1, Z1, weights1=None, "
     "weight_type=None,\n"
     ...
     "function, " XI_CHAR "(" RP_CHAR ", " PI_CHAR ") or wp(" RP_CHAR
     "). Pairs which are separated\n"
    ...
    {"countpairs_wp", (PyCFunction)countpairs_countpairs_wp, METH_VARARGS | METH_KEYWORDS,
     "countpairs_wp(boxsize, pimax, nthreads, binfile, X, Y, Z, weights=None, weight_type=None, "
     "verbose=False,\n"
     ...
     "\n"
     "If ``weights`` are provided and ``weight_type`` is \"pair_product\", then a weighted "
     "correlation function is returned.\n\n"
     ...
     "   Calculations are done in the precision of the supplied arrays.\n"
     "\n"
     "weights : array-like, real (float/double), shape (n_particles,) or "
     "(n_weights_per_particle,n_particles), optional\n"
     ...
     "   ``rpavg`` will be set to 0.0 for all bins; similarly for ``weight_avg``. ``wp`` contains "
     "the projected\n"
    ...
    {"countpairs_xi", (PyCFunction)countpairs_countpairs_xi, METH_VARARGS | METH_KEYWORDS,
     "countpairs_xi(boxsize, nthreads, binfile, X, Y, Z, weights=None, weight_type=None, "
     "verbose=False,\n"
     "              output_ravg=False, xbin_refine_factor=2, ybin_refine_factor=2,\n"
     "              zbin_refine_factor=1, max_cells_per_dim=100, c_api_timer=False, isa=-1)\n"
     "\n"
     ...
     "   ``ravg`` will be set to 0.0 for all bins; similarly for ``weightavg``. ``xi`` contains "
     "the projected\n"
     "   correlation function while ``npairs`` contains the number of unique pairs\n"
    ...
    {"countpairs_s_mu", (PyCFunction)countpairs_countpairs_s_mu, METH_VARARGS | METH_KEYWORDS,
     "countpairs_s_mu(autocorr, nthreads, binfile, mu_max, nmu_bins, X1, Y1, Z1, weights1=None, "
     "weight_type=None,\n"
     "                periodic=True, X2=None, Y2=None, Z2=None, weights2=None, verbose=False,\n"
     ...
     "   ranging from [0, ``mu_max``) *per* spatial bin. If ``output_savg`` is not set, then "
     "``savg``\n"
...
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT,       "_countpairs",     module_docstring,
    sizeof(struct module_state), module_methods,    NULL,
    _countpairs_traverse,        _countpairs_clear, NULL};
...
// of this file
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "iisO!O!O!|O!O!O!O!O!bbdbbbbhbis", kwlist, &autocorr, &nthreads, &binfile,
          &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj, &PyArray_Type, &z1_obj, &PyArray_Type,
          &weights1_obj, &PyArray_Type, &x2_obj, &PyArray_Type, &y2_obj, &PyArray_Type, &z2_obj,
          &PyArray_Type, &weights2_obj, &(options.periodic), &(options.verbose), &(options.boxsize),
          &(options.need_avg_sep), &xbin_ref, &ybin_ref, &zbin_ref, &(options.max_cells_per_dim),
    ...
    char msg[1024];
    int len = snprintf(
        msg, 1024, "ArgumentError: In DD> Could not parse the arguments. Input parameters are: \n");
  ...
  */
  ...
  int status = countpairs(ND1, X1, Y1, Z1, ND2, X2, Y2, Z2, nthreads, autocorr, binfile, &results,
                          &options, &extra);
...
static PyObject *countpairs_countpairs_rp_pi(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "iidsO!O!O!|O!O!O!O!O!bbdbbbbhbis", kwlist, &autocorr, &nthreads, &pimax,
          &binfile, &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj, &PyArray_Type, &z1_obj,
          &PyArray_Type, &weights1_obj, &PyArray_Type, &x2_obj, &PyArray_Type, &y2_obj,
          &PyArray_Type, &z2_obj, &PyArray_Type, &weights2_obj, &(options.periodic),
          &(options.verbose), &(options.boxsize), &(options.need_avg_sep), &xbin_ref, &ybin_ref,
          &zbin_ref, &(options.max_cells_per_dim), &(options.c_api_timer),
    ...
    int len = snprintf(
        msg, 1024,
        "ArgumentError: In DDrppi> Could not parse the arguments. Input parameters are: \n");
  ...
  double c_api_time = 0.0;
  int status = countpairs_rp_pi(ND1, X1, Y1, Z1, ND2, X2, Y2, Z2, nthreads, autocorr, binfile,
                                pimax, &results, &options, &extra);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  NPY_END_THREADS;
...
static PyObject *countpairs_countpairs_wp(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  PyObject *module = NULL;    // need not be used -> setting to NULL so any attempts to dereference
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "ddisO!O!O!|O!sbbbbbhbbi", kwlist, &boxsize, &pimax, &nthreads, &binfile,
          &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj, &PyArray_Type, &z1_obj, &PyArray_Type,
          &weights1_obj, &weighting_method_str, &(options.verbose), &(options.need_avg_sep),
          &xbin_ref, &ybin_ref, &zbin_ref, &(options.max_cells_per_dim), &(options.c_api_timer),
          &(options.c_cell_timer), &(options.instruction_set))
    ...
    char msg[1024];
    int len = snprintf(
        msg, 1024, "ArgumentError: In wp> Could not parse the arguments. Input parameters are: \n");
  ...
  if (x1_array == NULL || y1_array == NULL || z1_array == NULL) {
    ...
    char msg[1024];
    snprintf(msg, 1024,
             "TypeError: In %s: Could not convert input array to allowed floating point types "
             "(doubles or floats). Are you passing numpy arrays?",
             __FUNCTION__);
    perror(NULL);
    countpairs_error_out(module, msg);
  ...
  double c_api_time = 0.0;
  int status =
      countpairs_wp(ND1, X1, Y1, Z1, boxsize, nthreads, binfile, pimax, &results, &options, &extra);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  NPY_END_THREADS;
  ...
  for (int i = 1; i < results.nbin; i++) {
    ...
    const double rpavg = results.rpavg[i];
    const double weight_avg = results.weightavg[i];
    item = Py_BuildValue("(ddddkd)", rlow, results.rupp[i], rpavg, results.wp[i], results.npairs[i],
                         weight_avg);
    PyList_Append(ret, item);
    Py_XDECREF(item);
    rlow = results.rupp[i];
  ...
  if (options.c_cell_timer) {
    ...
    for (int i = 0; i < options.totncells_timings; i++) {
      PyObject *item = Py_BuildValue("(kkkiii)", t->N1, t->N2, t->time_in_ns, t->first_cellindex,
                                     t->second_cellindex, t->tid);
...
static PyObject *countpairs_countpairs_xi(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "disO!O!O!|O!sbbbbbhbi", kwlist, &boxsize, &nthreads, &binfile,
          &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj, &PyArray_Type, &z1_obj, &PyArray_Type,
          &weights1_obj, &weighting_method_str, &(options.verbose), &(options.need_avg_sep),
          &xbin_ref, &ybin_ref, &zbin_ref, &(options.max_cells_per_dim), &(options.c_api_timer),
    ...
    char msg[1024];
    int len = snprintf(
        msg, 1024, "ArgumentError: In xi> Could not parse the arguments. Input parameters are: \n");
  ...
  if (x1_array == NULL || y1_array == NULL || z1_array == NULL) {
    ...
    char msg[1024];
    snprintf(msg, 1024,
             "TypeError: In %s: Could not convert to array of allowed floating point type (doubles "
             "or floats). Are you passing numpy arrays?",
             __FUNCTION__);
    countpairs_error_out(module, msg);
    Py_RETURN_NONE;
  }
  ...
  double c_api_time = 0.0;
  int status =
      countpairs_xi(ND1, X1, Y1, Z1, boxsize, nthreads, binfile, &results, &options, &extra);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  NPY_END_THREADS;
  ...
  for (int i = 1; i < results.nbin; i++) {
    ...
    const double weight_avg = results.weightavg[i];
    item = Py_BuildValue("(ddddkd)", rlow, results.rupp[i], ravg, results.xi[i], results.npairs[i],
                         weight_avg);
    PyList_Append(ret, item);
    Py_XDECREF(item);
    rlow = results.rupp[i];
...
static PyObject *countpairs_countpairs_s_mu(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "iisdiO!O!O!|O!O!O!O!O!bbdbbbbhbis", kwlist, &autocorr, &nthreads, &binfile,
          &mu_max, &nmu_bins, &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj, &PyArray_Type,
          &z1_obj, &PyArray_Type, &weights1_obj, &PyArray_Type, &x2_obj, &PyArray_Type, &y2_obj,
          &PyArray_Type, &z2_obj, &PyArray_Type, &weights2_obj, &(options.periodic),
          &(options.verbose), &(options.boxsize), &(options.need_avg_sep), &xbin_ref, &ybin_ref,
          &zbin_ref, &(options.max_cells_per_dim), &(options.c_api_timer),
          &(options.instruction_set), &weighting_method_str)
    ...
    int len = snprintf(
        msg, 1024,
        "ArgumentError: In DDsmu> Could not parse the arguments. Input parameters are: \n");
  ...
  double c_api_time = 0.0;
  int status = countpairs_s_mu(ND1, X1, Y1, Z1, ND2, X2, Y2, Z2, nthreads, autocorr, binfile,
                               mu_max, nmu_bins, &results, &options, &extra);
  if (options.c_api_timer) {
    c_api_time = options.c_api_time;
  }
  NPY_END_THREADS;
  ...
  for (int i = 1; i < results.nsbin; i++) {
    ...
    for (int j = 0; j < results.nmu_bins; j++) {
      ...
      const double weight_avg = results.weightavg[bin_index];
      item = Py_BuildValue("(ddddkd)", smin, smax, savg, (j + 1) * dmu, results.npairs[bin_index],
                           weight_avg);
      PyList_Append(ret, item);
      Py_XDECREF(item);
    }
...
static PyObject *countpairs_countspheres_vpf(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "diiikO!O!O!|bbdbbbhbi", kwlist, &rmax, &nbin, &nc,
                                   &num_pN, &seed, &PyArray_Type, &x1_obj, &PyArray_Type, &y1_obj,
                                   &PyArray_Type, &z1_obj, &(options.periodic), &(options.verbose),
                                   &(options.boxsize), &xbin_ref, &ybin_ref, &zbin_ref,
                                   &(options.max_cells_per_dim), &(options.c_api_timer),
                                   &(options.instruction_set))
    ...
    int len =
        snprintf(msg, 1024,
                 "ArgumentError: In vpf> Could not parse the arguments. Input parameters are: \n");
  ...
  double c_api_time = 0.0;
  int status =
      countspheres(ND1, X1, Y1, Z1, rmax, nbin, nc, num_pN, seed, &results, &options, NULL);
